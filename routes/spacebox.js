...
router.delete('/ideas/:id', requirePermission('spacebox', 'editar'), async (req, res) => {
  try {
    const { rowCount } = await pool.query(
      "DELETE FROM ideas WHERE id = $1 AND business = 'spacebox'",
      [req.params.id]
    );
    if (!rowCount) return res.status(404).json({ error: 'Idea no encontrada' });
...
router.get('/history', requirePermission('spacebox', 'ver'), async (req, res) => {
  try {
    ...
    let where = `ch.business = 'spacebox'`;
    const params = [];
    let idx = 1;
    ...
    const usersRes = await pool.query(
      `SELECT DISTINCT u.id, u.name FROM content_history ch JOIN users u ON ch.user_id = u.id WHERE ch.business = 'spacebox' ORDER BY u.name`
    );
...
router.delete('/history/:id', requirePermission('spacebox', 'editar'), async (req, res) => {
  try {
    ...
    const p = isAdmin ? [req.params.id, 'spacebox'] : [req.params.id, req.user.id, 'spacebox'];
...
router.post('/history/bulk-delete', requirePermission('spacebox', 'editar'), async (req, res) => {
  try {
    ...
    const p = isAdmin ? [ids, 'spacebox'] : [ids, req.user.id, 'spacebox'];
    const { rowCount } = await pool.query(q, p);
